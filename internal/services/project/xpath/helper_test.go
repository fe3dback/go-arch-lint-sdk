...
func TestHelper_FindProjectFiles(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.gold, func(t *testing.T) {
			...
			// assert
			wantContent := make(map[arch.PathAbsolute]any)
			...
			gotContent := make(map[arch.PathAbsolute]any)
			for _, dst := range got {
...
// nolint
func createQueryFromDocMeta(meta goldenMeta) arch.FileQuery {
	var queryType arch.FileMatchQueryType

	switch meta.QueryType {
	case "files":
		queryType = arch.FileMatchQueryTypeOnlyFiles
	case "directories":
		queryType = arch.FileMatchQueryTypeOnlyDirectories
	case "all":
		queryType = arch.FileMatchQueryTypeAll
	default:
	...
	if filepath.IsAbs(meta.Query) {
		if isGlob {
			path = arch.PathAbsoluteGlob(meta.Query)
		} else {
			path = arch.PathAbsolute(meta.Query)
		}
	} else {
		if isGlob {
			path = arch.PathRelativeGlob(meta.Query)
		} else {
			path = arch.PathRelative(meta.Query)
		}
	}
	...
	var excludeDirs []arch.PathRelative
	if len(meta.ExcludeDirs) > 0 {
		for _, dir := range meta.ExcludeDirs {
			excludeDirs = append(excludeDirs, arch.PathRelative(dir))
		}
	}

	var excludeRegexp []arch.PathRelativeRegExp
	if len(meta.ExcludeFilesRegexp) > 0 {
		for _, fileRegexp := range meta.ExcludeFilesRegexp {
			excludeRegexp = append(excludeRegexp, arch.PathRelativeRegExp(fileRegexp))
		}
	}

	var excludeFiles []arch.PathRelative
	if len(meta.ExcludeFiles) > 0 {
		for _, file := range meta.ExcludeFiles {
			excludeFiles = append(excludeFiles, arch.PathRelative(file))
		}
	}

	return arch.FileQuery{
		Path:               path,
		WorkingDirectory:   arch.PathRelative(meta.WorkDirectory),
		Type:               queryType,
